...
- Present 推荐主题 and 报告模板 as sibling tabs in the same content area for 企业数据分析; opening 报告模板 replaces the recommendation content instead of adding a second stacked section.
...
- The gallery itself shows compact visual thumbnails. Clicking the card body selects that template and writes the default 上下滑动 recommendation prompt; the separate preview action opens the report detail modal, where the 上下滑动/分页切换 mode switch lives.
...
- Confirming a report template in its preview modal writes `生成【模板名称】风格的分析报告，报告支持【当前预览模式】，包含现状概览、表现分析、结论建议。` into the 业务分析 composer, using only the just-selected template name and the mode currently shown in the modal; do not include the selected topic. Clicking a template card directly uses 上下滑动 by default. The populated prompt enables sending.
- When sending, parse the prompt again: a recognized template name selects the corresponding template, 上下滑动/分页切换 selects the report behavior, no style description keeps the current default template, and additional text beyond the preset prompt follows the custom-style path while retaining the selected template as its base.
